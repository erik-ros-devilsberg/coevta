...
/** Ids for contacts created offline, before the server has assigned a real one. */
...
export function createOutbox({ kv }) {
	 ...
	 * contact whose create has not synced would target an id the server has
	...
	async function enqueue(op) {
		...
		// edits queue separately and are repointed by remapContactId once the
		// create comes back with a real id.
		const pendingFor = queued.filter((existing) => existing.contactId === op.contactId && !existing.sending);
		...
		if (op.type === 'update' && create) {
			// The contact does not exist server-side yet — fold the edit into the
	 ...
	 */
	async function remapContactId(from, to) {
		for (const op of await list()) {
			if (op.contactId === from) {
				await kv.set(op.id, { ...op, contactId: to });
			}
		}
	}
	...
	async function pendingIds() {
		return new Set((await list()).map((op) => op.contactId));
	}

	return { enqueue, list, remove, markSending, remapContactId, count, pendingIds };
}
