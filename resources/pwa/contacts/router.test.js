...
describe('scope', () => {
	it('is based at /contacts/, so every route stays inside the service worker scope', () => {
...
describe('auth guard', () => {
	...
	it('guards the detail and form routes too', async () => {
		isAuthenticatedMock.mockReturnValue(false);

		expect((await visit('/new')).path).toBe('/login');
		expect((await visit('/abc')).path).toBe('/login');
		expect((await visit('/abc/edit')).path).toBe('/login');
	});
