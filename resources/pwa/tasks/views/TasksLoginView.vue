<script setup>
// Login lives inside the PWA's own scope (/tasks/login) on purpose: a navigation
// outside the service worker scope would kick an installed app out into a
// browser tab. The token itself is shared with any other app on the origin, so
// signing in here signs you in everywhere.
...
async function submit() {
	...
	if (!online.value) {
		error.value = 'Signing in needs a connection. Your saved tasks are still readable offline.';
		return;
	}
...
<template>
	<main class="container app-view">
		...
		<!-- A plain anchor, deliberately leaving the app: one account backs all
		     ...
		     this router's /tasks/ base, and on an installed app it opens a
